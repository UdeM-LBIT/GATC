      ...
      if(processID == 0)
	{
	  ...
	  if(adef->useMultipleModel && !filexists(noDupModels) && countUndeterminedColumns)
	      ...
	      for(i = 0; i < tr->NumberOfModels; i++)
		{
		  ...
		  if(modelStillExists)
		    {	  
		      char *protModels[11] = {"DAYHOFF", "DCMUT", "JTT", "MTREV", "WAG", "RTREV", "CPREV", "VT", "BLOSUM62", "MTMAM", "LG"};
		      int k = 1;
		      int lower, upper;
		      int parts = 0;
  ...
  if(processID == 0)
      ...
      for(model = 0; model < tr->NumberOfModels; model++)
	{
	  ...
	  switch(tr->partitionData[model].dataType)
	    ...
	    case AA_DATA:
	      {
		char *protStrings[11] = {"DAYHOFF", "DCMUT", "JTT", "MTREV", "WAG", "RTREV", "CPREV", "VT", "BLOSUM62", "MTMAM", "LG"};
		char basesPROT[20] = {'A', 'R', 'N' , 'D', 'C', 'Q','E','G','H','I','L','K','M','F','P','S','T','W','Y','V'};		

		assert(tr->partitionData[model].protModels >= 0 && tr->partitionData[model].protModels < 11);		
